...
class CreateSprite {
    ...
    retinaTemplate (data, retina) {
        ...
        console.log(image, retina)
        let { fontSize } = this
        let perNames = []
        let shared = `.${prefix} { background-image: url(I);background-size:Xrem Yrem}`
            .replace('I', image)
            .replace('X', total_width / fontSize)
            .replace('Y', total_height / fontSize)
        let perSprite = data.sprites.map((sprite) => {
            ...
            return `.${prefix}-N {display:inline-block; width: Wrem; height: Hrem; background-position: Xrem Yrem;@extend .${prefix} }`
                .replace('N', name)
                .replace('W', sprite.width / fontSize)
                .replace('H', sprite.height / fontSize)
                .replace('X', sprite.offset_x / fontSize)
                .replace('Y', sprite.offset_y / fontSize)
        }).join('\n')
        let mixin = ''
